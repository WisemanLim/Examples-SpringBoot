...
public class BoardController {
    ...
    @PostMapping("/save")
    public String save(@ModelAttribute BoardVO boardVO) throws IOException {
    ...
    public String findById(@PathVariable Long id, Model model,
        ...
        /* 댓글 목록 가져오기 */
        List<CommentVO> commentVOList = commentService.findAll(id);
    ...
    public String updateForm(@PathVariable Long id, Model model) {
        BoardVO boardVO = boardService.findById(id);
        model.addAttribute("boardUpdate", boardVO);
        return "update";
    }

    @PostMapping("/update")
    public String update(@ModelAttribute BoardVO boardVO, Model model) {
        BoardVO board = boardService.update(boardVO);
...
//        pageable.getPageNumber();
        Page<BoardVO> boardList = boardService.paging(pageable);
        int blockLimit = 3;
